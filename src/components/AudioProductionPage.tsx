...
export const AudioProductionPage = () => {
  const { mvData, updateAudioChapter, updateNarratorVoiceProfile, updateMVInfoAsset, updateMVInfoAudioTiming, updateMVInfoAudioText, updateMVInfoAudioTexts, lockAudioTimeline, upgradeCurrentProjectAudioPlan } = useGlobalSettings();
  ...
  return <div className="space-y-6">
    ...
    <section className="glass-card rounded-xl border border-emerald-300/20 bg-emerald-500/5 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><AudioLines className="text-emerald-300" /><h2 className="text-2xl font-bold text-white">声音制作</h2></div><p className="mt-2 text-sm text-gray-400">千问 3 TTS 负责配音和音色一致性；MiniMax Music 3 只负责纯器乐背景配乐。</p><p className="mt-1 text-xs text-emerald-200/70">最终 Drive Audio：配音 100% + 配乐 18% · 状态 {plan.alignment_status}</p></div><div className="flex max-w-2xl flex-wrap justify-end gap-2"><input ref={lrcInputRef} type="file" accept=".lrc,text/plain" className="hidden" onChange={(event) => importLrc(event.target.files?.[0])} /><button type="button" disabled={Boolean(busy)} onClick={downloadLrc} className="flex items-center gap-2 rounded border border-emerald-300/40 bg-emerald-500/10 px-3 py-2 text-xs font-bold text-emerald-200 disabled:opacity-50"><Download size={14} />导出 LRC</button><button type="button" disabled={Boolean(busy)} onClick={() => lrcInputRef.current?.click()} className="flex items-center gap-2 rounded border border-emerald-300/40 bg-emerald-500/10 px-3 py-2 text-xs font-bold text-emerald-200 disabled:opacity-50"><Upload size={14} />导入翻译 LRC</button><button type="button" disabled={Boolean(busy)} onClick={openVoicePlaylist} className="flex items-center gap-2 rounded border border-cyan-300/40 bg-cyan-500/10 px-3 py-2 text-xs font-bold text-cyan-200 disabled:opacity-50"><Headphones size={14} />聆听全部配音</button><button type="button" disabled={Boolean(busy)} onClick={downloadAllVoices} className="flex items-center gap-2 rounded border border-fuchsia-300/40 bg-fuchsia-500/10 px-3 py-2 text-xs font-bold text-fuchsia-200 disabled:opacity-50">{busy === 'download-voices' ? <Loader2 size={14} className="animate-spin" /> : <PackageOpen size={14} />}下载所有配音</button><button type="button" disabled={Boolean(busy)} onClick={lock} className="flex items-center gap-2 rounded border border-cyan-300/40 bg-cyan-500/10 px-3 py-2 text-xs font-bold text-cyan-200 disabled:opacity-50"><LockKeyhole size={14} />锁定声音时间线</button></div></div>
