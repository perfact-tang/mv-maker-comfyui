...
- For v4 `qwen3-tts-audio-first` shots, declare `generation_plan.audio_mode` as `drive-audio`. `<Audio 1>` is the finalized Qwen3 TTS voice plus the optional low-level score generated by the chapter's selected music workflow. State that lip movement, gestures, cuts, and action timing follow it exactly. Do not ask H3 to regenerate dialogue or music.
