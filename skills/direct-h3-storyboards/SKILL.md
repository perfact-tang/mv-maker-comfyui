...
把源文本转换为可执行的导演方案与 `mv-maker-project` v3 JSON。先锁定统一视觉系统和角色参考板，再写分镜；不得让各角色自行选择画风。
...
确定源类型、内容形式、语言、交付物、成片画幅、视频模型、图片工作流、可用片长、音频归属和用户是否提供时间点。未指定时使用 MiniMax H3、16:9、片长 `[5, 10, 15]`，图片工作流优先 `Z-Image-Turbo`，备选 `Krea2 Turbo`。短剧或没有外部主音频的声画项目默认使用 `native-audio`，让 H3 从中文提示词生成对白、环境声、动作声与音乐并写入导出视频；只有用户明确要求静音时才使用 `no-audio`。已有逐镜头音频驱动时使用 `drive-audio`，仅作音色或节奏参考时使用 `reference-audio`。
...
- `Ref2VA`：人物身份、服装、场景或风格参考比精确关键帧更重要；必须声明恰好两个参考槽。

5、10、15 秒分别对应 141、260、379 帧。使用能清楚表达动作的最短时长，结尾补到 5 秒边界，不得截断源文。

人物出现的镜头必须引用已锁定角色板，不得重新描述一种新画风。多人镜头优先用角色板作为 Ref2VA 身份参考；若参考槽不足，拆镜头而不是牺牲人物一致性。

### 6. 写中文模型提示词

图像与 H3 提示词正文全部使用中文，并保持 `references/h3-routing.md` 规定的字段顺序。H3 固定字段名可保留英文，字段值使用中文。对白只放在 `<d>[Chinese] ...</d>` 等协议块内。完成结构后不得追加游离风格段落。

`source_text` 使用准确原文。`lyrics` 对音乐保留准确歌词，对其他形式填写必要对白/旁白；无对白使用 `(No dialogue)` 以兼容现有应用。

### 7. 验证与交付

从 `assets/storyboard-template.json` 开始，替换所有占位符，写出完整 `mv-maker-project` v3 JSON。运行：
