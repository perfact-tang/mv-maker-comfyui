...
人物展示页会直接使用 `description` 作为首帧和视频生成提示词。描述可在页面内修改，AI 生成结果会写回 `characters[].generated_assets`。如果已有角色图，也可以在对应人物卡点击“上传人物图片”，直接选择 PNG、JPG、WebP 等不超过 40 MB 的本地图片。上传时会在浏览器内自动将长边限制为最多 1600px，并在需要时转换为 WebP、逐步压缩到约 450 KB，以避免项目自动保存触发浏览器存储额度错误；上传完成后页面会显示优化前后的文件大小。上传图与 AI 生成图使用相同的 `characters[].generated_assets.image` 字段，因此会正常参与分镜引用、项目保存和再次载入。整体视频方向支持横版 `736×416` 与竖版 `416×736`，该设定同时作用于人物展示和分镜制作。

分镜制作中的本地图片入口使用同一套自动优化逻辑，包括 T2I 首帧参考图、FL2VA 目标尾帧、逐镜头 Ref2VA 本地参考图，以及旧项目兼容模式下的全局 H3 参考图。压缩只优化图片资源，不改变镜头提示词、`<Picture 1/2>` 声明、人物来源绑定或生成模式。
...
v4 项目可在 `project.director_plan.audio_plan` 中声明 `qwen3-tts-audio-first`。人物展示通过下拉框提供两种固定音色创建方式：文本定义使用 Voice Design；参考声音使用 Voice Clone + ASR。参考声音既可以上传本地文件，也可以授权麦克风后直接在网页录制；录音停止后会自动转换为创建参考、测量实长并标记来源。两种方法的生成结果都会保存为人物最终固定音色。声音制作选择人物或旁白后只建立绑定，点击“生成配音”时才克隆最终固定音色。
